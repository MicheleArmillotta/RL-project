...
def shaped_reward(prev_state, next_state,agent_idx,n_steps):
    ...
    #        reward += 0.1
            #print("DEBUG = > PRESO LA ZUPPA ALLO STEP: ", n_steps)
...
if __name__ == '__main__':
    ...
    agent0 = Agent(action_dim=env.action_space.n, batch_size=batch_size, 
                    ...
                    input_dim=env.observation_space.shape, gamma = 0.99, GAElambda=0.95, epsilon=0.2,entropy_coeff=0.01,critic_smoother= 1,agent_name="Agent0")
    
    agent1 = Agent(action_dim=env.action_space.n, batch_size=batch_size, 
                    learning_rate=alpha, epochs=n_epochs, 
                    input_dim=env.observation_space.shape, gamma = 0.99, GAElambda=0.95, epsilon=0.2,entropy_coeff=0.01, critic_smoother=1, agent_name="Agent1")
    n_games = 7000
