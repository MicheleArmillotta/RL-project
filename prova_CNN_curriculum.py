...
from PPO_clip_CNN import Agent
import os
...
class GeneralizedOvercooked:
    ...
    def add_layout(self, avg_score):
        if avg_score >= 80 and self.layout_episode >= 300:
    ...
    def reset(self, avg_score):
        if len(self.curriculum) >= 4 and avg_score >= 80: #true random testing
            idx = random.randint(0, len(self.envs))
        else:
...
if __name__ == '__main__':
    ...
    agent0 = Agent(action_dim=env.action_space.n, batch_size=batch_size, 
                    ...
                    input_dim=env.observation_space.shape, gamma = 0.99, GAElambda=0.95, epsilon=0.2,entropy_coeff=0.05,critic_smoother= 0.5,eps = 0.2,agent_name="Agent0")
    
    agent1 = Agent(action_dim=env.action_space.n, batch_size=batch_size, 
                    learning_rate=alpha, epochs=n_epochs, 
                    input_dim=env.observation_space.shape, gamma = 0.99, GAElambda=0.95, epsilon=0.2,entropy_coeff=0.05, critic_smoother=0.5, eps = 0.2,agent_name="Agent1")
    n_games = 20000 
    best_score = 0
    score_history = []
    stack_obs = []
    ...
    for i in range(n_games):
        ...
        base_mdp = env.get_base_mdp()
        #print("DEBUG => OBSERVATION RESET", observation)

        observation_state = observation["overcooked_state"]
        agent0.reset_tensor_stack()
        agent1.reset_tensor_stack()
        ...
        while not done:
            ...
            obs_tensor = agent0.state_to_tensor(observation_state,base_mdp) # 15*15*40
            action0, old_prob0 = agent0.generate_action(obs_tensor)
            S_value0 = agent0.predict_value(obs_tensor)
            action1, old_prob1= agent1.generate_action(obs_tensor)
            S_value1 = agent1.predict_value(obs_tensor)

            joint_action = (action0, action1)
            observation_, reward, done, info = env.step(joint_action)
            #visualizer.display_rendered_state(observation_["overcooked_state"],grid = base_mdp.terrain_mtx,img_path=img_path)
            n_steps += 1
            ...
            score += reward
            agent0.remember(obs_tensor, action0, (reward+shp_reward0),S_value0,old_prob0,done)
            agent1.remember(obs_tensor, action1, (reward + shp_reward1),S_value1,old_prob1,done)
            if n_steps % N == 0:
                next_value_tensor = agent0.state_to_tensor(observation_["overcooked_state"],base_mdp)
                S_value_next0 = agent0.predict_value(next_value_tensor)
                agent0.remember_nextS(S_value_next0)
                agent0.train()
                S_value_next1 = agent1.predict_value(next_value_tensor)
